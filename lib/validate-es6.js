(function(){
   ...
   */
  const Validate = function (_fields) {
  ...
  Validate.prototype._handle = function (_field_name, _rules) {
    if (typeof this[_rules.type] === 'function') {
      ...
      console.warn("The function '"+_rules.type+"' does not exists.")
    }
  }
  ...
  Validate.prototype._validateStringLength = function (_field_name, _rules) {
    const value = _rules.value.length
    let validation_status = true
  ...
  Validate.prototype.integer = function (_field_name, _rules) {
    const pattern = /^[0-9]*$/
    const value = parseInt(_rules.value)
  ...
  Validate.prototype.money = function (_field_name, _rules) {
    ...
    const value = parseFloat(_rules.value.replace('.', '').replace(',', '.'))
  ...
  Validate.prototype.cpf = function (_field_name, _rules) {
    const strCpf = _rules.value.replace(/[^\d]+/g, '')
    let validation_status = true,
        sum = 0,
        rest
    if (
  ...
  Validate.prototype.cnpj = function (_field_name, _rules) {
    const cnpj = _rules.value.replace(/[^\d]+/g, '')
    let validation_status = true
    let str_length = cnpj.length - 2,
  ...
  Validate.prototype.cpfOrCnpj = function (_field_name, _rules) {
    const value = _rules.value.replace(/[^\d]+/g, '')
    const cpf_length = 11
   ...
   * @return {boolean}             if the field is valid or not
   */
  Validate.prototype.email = function (_field_name, _rules) {
   ...
   * @return {boolean}             if the field is valid or not
   */
  Validate.prototype.zipcode = function (_field_name, _rules) {
    const pattern = /[^\d]+/g
    let zipcode_length = 0
    ...
    const value = _rules.value.replace(pattern, '')
